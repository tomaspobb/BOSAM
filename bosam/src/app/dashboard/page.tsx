  ...
  return (
    <main className="py-2">
      <div className="card-bosam mb-3 d-flex justify-content-between align-items-center flex-wrap">
        <h1 className="m-0 fw-bold" style={{color:'var(--bosam-black)'}}>Bosam</h1>
        <div className="d-flex gap-2">
          <a className="btn btn-outline-dark" href="#">+ Cliente</a>
          <button className="btn-bosam">+ Proyecto</button>
        </div>
      </div>
      ...
      <div className="card-bosam mb-3">
        ...
        <ul className="mb-0">
          ...
          {clients.map(c=> <li key={c._id}><strong>{c.name}</strong> <span className="text-muted">({c.code})</span></li>)}
        </ul>
      </div>
