...
export default function NuevoProyectoButton({ onCreated }: { onCreated?: () => void }) {
  ...
  // liberar objectURLs al cerrar el modal
  useEffect(() => {
    if (!show) return;
    return () => {
      services.forEach(s =>
  ...
  function addCustomService() {
    setServices(p => [
      ...p,
      { label: 'Servicio', unitPrice: 0, qty: 1, subtotal: 0, files: [] },
    ]);
  }
  ...
  function updatePrice(i: number, price: number) {
    setServices(p => {
      const c = [...p];
      const val = Math.max(0, Number.isFinite(price) ? price : 0);
      c[i].unitPrice = val;
      c[i].subtotal = val * (c[i].qty || 1);
      return c;
    });
  }
  ...
  function updateQty(i: number, qty: number) {
    setServices(p => {
      ...
      c[i].qty = q;
      c[i].subtotal = (c[i].unitPrice || 0) * q;
      return c;
    });
  }
  ...
  function validateFiles(list: FileList | null) {
    ...
    for (const f of Array.from(list)) {
      if (f.size > max) {
        alert(`"${f.name}" supera ${MAX_FILE_MB} MB`);
        return false;
      }
    }
    return true;
  ...
  function onPickFiles(serviceIdx: number, list: FileList | null) {
    ...
    setServices(prev => {
      ...
      for (const file of Array.from(list)) {
        ...
        if (exists.has(key)) continue; // no duplicados
        toAdd.push({
          file,
  ...
  function removeFile(serviceIdx: number, fileIdx: number) {
    ...
    setServices(prev => {
      ...
      cp[serviceIdx] = {
        ...cp[serviceIdx],
        files: cp[serviceIdx].files.filter((_, i) => i !== fileIdx),
      };
      return cp;
    });
  }
  ...
  async function save() {
    ...
    try {
      ...
      // igual que tu versión original: files[0], files[1], etc
      services.forEach((s, si) =>
        s.files.forEach(lf => {
          fd.append(`files[${si}]`, lf.file);
        })
      );
      ...
      services.forEach(s =>
        s.files?.forEach(f => URL.revokeObjectURL(f.previewUrl))
      );
    ...
    <>
      ...
      <Modal show={show} onHide={() => setShow(false)} size="lg" centered>
          ...
          {services.map((s, idx) => (
            ...
            >
              ...
              <div className="mt-3">
                ...
                {/* input simple y directo, sin refs ni clicks raros */}
                <input
                  type="file"
                  multiple
                ...
                {/* lista/preview de archivos seleccionados */}
                <div
        ...
        <div className="d-flex justify-content-end gap-2 p-3 pt-0">
          ...
          <button className="btn-bosam" onClick={save} disabled={saving}>
